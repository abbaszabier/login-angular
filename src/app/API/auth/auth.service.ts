...
export class AuthService extends AbstractApiService<any> {
  ...
  async post_login(data: { phone: string; password: string }, param?: any) {
    return Entity_User.load(async () => {
      ...
      if (res?.access_token) {
        this.setLogin(true);
        this.helper.setStorage(environment.tokenName, {
          token: res.access_token,
          expired: Date.now() + (res.expires_in ?? 0) * 1000,
        });

        const userRes = await this.get('/users');
        const user = userRes?.data ?? userRes;
        this.helper.setStorage('users', user);
        Entity_User.set(user);
